...
\frame
{
  ...
  \begin{Definition}
    ...
    \begin{description}
    \item<1->[\alert<1| handout:0| trans:0>{$\DOF$}]
      if $L$ is accepted by a deterministic overhead-free machine with
      input alphabet~$\Sigma$, 
    \item<2->[\alert<2| handout:0| trans:0>{$\DOFpoly$}]
      if $L$ is accepted by a deterministic overhead-free machine with
      input alphabet~$\Sigma$ in polynomial time.
    \item<3->[\alert<3| handout:0| trans:0>{$\NOF$}]
      is the nondeterministic  version of $\DOF$, 
    \item<4->[\alert<4| handout:0| trans:0>{$\NOFpoly$}]
...
\frame
{
  \frametitle{Continued Review of Linear Grammars}
...
\frame
{
  ...
  The proof is based on the fact that every context-free
  language can be accepted by a nondeterministic almost-overhead-free
