...
\end{verbatim}
  ...
  \item
    ...
    always desirable) or that you need to use $\epsilon$-\LaTeX.
    ...
    If you use $\epsilon$-(pdf)\LaTeX, the writing and reading of files
    is not done. Instead the special primitive |\scantokens| is used
    to achieve the desired effect. This is a cleaner solution and
    avoids problems with characters outside the standard
    \textsc{ascii} range.
  ...
  \item
    ...
    is only usedful in conjunction with the |fragile| option (but it
    ...
    contents. Normally, the frame ends when the a line reading
...
|\end{frame}|. Anything really complex is likely to fail.
