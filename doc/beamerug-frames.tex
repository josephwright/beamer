...
\begin{environment}{{frame}\sarg{overlay specification}%
  ...
  changes the character codes, unless the |containsverbatim| options
  is given, see also Section~\ref{section-verbatim}. 
...
\end{verbatim}
  \item
    ...
    options |t| and |c|.
  \item
    \declare{|containsverbatim|\opt{|=|\meta{mode}}} tells \beamer\
    that the frame contains verbatim commands. In this case, different
    internal mechanisms are used to typeset the frame to ensure that
    inside the frame the character codes can be reset. The price of
    switching to another internal mechanism is that either you cannot
    use overlays or an external file needs to be written and read back
    (which is not always desirable).

    In detail, the following \meta{mode}s can be specified:
    \begin{itemize}
    \item
      \declare{|false|} just means that no character code changes
      occur in the frame. This is the default.
    \item
      \declare{|true|} indicates that character code changes (caused,
      for example, by a |verbatim|) occur inside the frame. In this
      case, only the first slide of the frame is typeset (unless all
      slides are suppressed by the \meta{overlay specification}).
    \item
      \declare{|overlays|} indicates that the character codes change
      (because of, say, a |verbatim|) and overlays are needed for this
      frame. In this case, the contents of the frame is written to a
      special file called \meta{jobname}|.vrb|. Then, the frame is
      started anew and the content of this file is read back. Since,
      upon reading of a file, the character codes can be modified,
      this allows you to use both verbatim text and overlays.

      The original \meta{options} are passed to the frame upon
      reading from disk, except for the |containsverbatim| option, of
      course.
    \end{itemize}    
  \item
