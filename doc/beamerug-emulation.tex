...
class for preparing your presentations. The intention of these layers
...
\item
  ...
  files. You should convert them to |.pdf| or to |.png| and adjust any
...
In the following, the effects of \prosper\ commands in \beamer\ are
listed.
...
\begin{command}{\dualslide\oarg{x}\oarg{y}\oarg{z}\marg{options}\marg{left
  ...
  and \meta{z} are ignored. Also, all \meta{options} are ignored,
...
\item
  There should not be commands in the preamble having to do with
...
In the following, the effects of \seminar\ commands in \beamer\ are
listed.
...
In the following, the effects of \foils\ commands in \beamer\ are
listed.
...
needs another class, like |seminar| or |prosper| to do the actual
...
whereas it may only be used in non-nested situations in \texpower. Since
...
\begin{command}{\stepwise\marg{text}}
  As in \texpower, this initiates text in which commands like |\step|
