...
% Copyright 2019-2026 by Joseph Wright, samcarter
%
...
\end{verbatim}
  ...
  \begin{element}{itemize items}\semiyes\no\no
    ...
    \begin{templateoptions}
      ...
      \itemoption{left triangle}{}
      Uses left pointing triangles as item markers. These can be used to replace the default triangles in a right-to-left context.   
    \end{templateoptions}
  \end{element}
...
\end{verbatim}
  ...
  \item
    \declare{|shadow|}\opt{|=|\meta{\underline{true} or false}} draws a shadow behind the box. Currently, this option only has an effect if used together with the |rounded| option, but that may change.
  \item
    \declare{|rounded=|\meta{true or false}} causes the borders of the box to be rounded off if there is a background installed. This command internally calls |beamerboxesrounded|. In this case, |colsep*| option will have no effect.
  \item
...
\begin{environment}{{beamerboxesrounded}\oarg{options}\marg{head}}
  ...
  \item
    \declare{|shadow|}\opt{|=|\meta{\underline{true} or false}}. If set to |true|, a shadow will be drawn.
  \item
...
\begin{command}{\footnote\sarg{overlay specification}\oarg{options}\marg{text}}
  Inserts a footnote into the current frame. Footnotes will always be shown on the current frame; they will never be ``moved'' to other frames. As usual, one can give a number as \meta{options}, which will cause the footnote to use that number. The \beamer\ class adds two additional options:
  \begin{itemize}
  \item
    \declare{|frame|} causes the footnote to be shown at the bottom of the frame. This is normally the default behavior anyway, but in minipages, columns and certain blocks it makes a difference. In a minipage, the footnote is usually shown as part of the minipage rather than as part of the frame.

    This option can also be set for the whole document with the |framefootnotes| class option:
        \begin{classoption}{framefootnotes}
        \end{classoption}
  \item
    \declare{|noframe|} is the antidote to the |frame| option. It ensures that the footnote is shown as part of the minipage even if the |framefootnotes| class option is used.
  \end{itemize}
