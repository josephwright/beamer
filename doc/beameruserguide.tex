...
\begin{package}{{CJK}}
  When using the |CJK| package, you should use the class option
  \declare{|CJK|}.
\end{package}
...
inserting graphics, like the command |\includegraphic|. However,
...
|\includegraphic| will put a copy of the graphic into the file
...
only if you use |pdflatex|): There, you must first \emph{declare}
...
the |.pdf| file. Furthermore, if you use |latex| instead of
|pdflatex|, the \textsc{pgf} package will automatically search
for a graphic file with the extension |.eps| instead of the
extensions appropriate for |pdflatex|.
...
extensions automatically (|.eps| for normal \LaTeX,
|.png|, |.jpg|, and |.pdf| for
|pdflatex|). 
...
\begin{command}{\nameslide\sarg{overlay specification}\marg{slide
  ...
  target of the name \meta{slide name} is also setup.
  \example
\begin{verbatim}
\frame{
  \nameslide<1>{slide1}
...
\begin{command}{\includeslide\oarg{options}\marg{slide name}}
  ...
  to |\pgfimage|, where the \meta{page of slide name} is read from the
  file \meta{filename}|.snm|. See the documentation of |pgf| for the
  exact effect of this option.
  \example
