...
of the \beamer's User's Guide, where a workaround is described. 


\subsection{Using Overlay Specifications}

The next frame is to show his main argument and is put the ``Results''
...
\begin{frame}
  ...
  \begin{block}{Open Questions}
    How many primes $p$ are there such that $p+2$ is also prime?
  \end{block}
\end{frame}
\end{verbatim}
...
\begin{frame}
  ...
  \item Open Questions
    \begin{itemize}
    \item How many primes $p$ are there such that $p+2$ is also prime?
    \end{itemize}
  \end{itemize}
\end{frame}
\end{verbatim}
...
\usetheme{Warsaw}
\end{verbatim}
By substituting other cities (he notices that the only thing these
cities seem to have in common is that in most of them the same person
...
He settles on the |Frankfurt| theme but decides that the whole thing
is still too colorful. He adds
\begin{verbatim}
\usecolortheme{seahorse}
\usecolortheme{rose} 
\end{verbatim}  
The result seems some more subdued.
